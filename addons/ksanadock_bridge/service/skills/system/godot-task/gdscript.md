...
- **Avoid `get_node` in Hot Loops**: Never call `$Node` or `get_node()` inside `_process` or `_physics_process`. Use `@onready var node = $Node` instead.
- **Signal Connectivity**: Use the modern `Callable` syntax: `node.signal_name.connect(_on_handler)`. Avoid string-based connections.
- **Type Safety**: Prefer `var x: int = 0` over `var x = 0`. This helps the compiler optimize and prevents bugs.
- **Node Cleanup**: Use `queue_free()` for nodes to ensure they are removed safely at the end of the frame.
- **Resource Management**: Use `load()` for resources that might not exist or are determined at runtime; use `preload()` only for essential, always-present resources (note: `preload` is not available in headless scene builders).
- **Group Communication**: Use `get_tree().call_group("group", "method")` for decoupling systems like "enemies" or "pickups".
- **Input Actions**: Always use `Input.is_action_pressed("action_name")` instead of hardcoding keys. Action names should be defined in the project's Input Map.
