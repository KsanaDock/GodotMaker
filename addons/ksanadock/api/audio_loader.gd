...
static func _get_player() -> AudioStreamPlayer:
	...
	_player.name = "KsanaDockAudioPlayer"
...
static func play(url: String) -> void:
	...
	http.request_completed.connect(func(result: int, code: int, headers: PackedStringArray, body: PackedByteArray):
		...
		if result != HTTPRequest.RESULT_SUCCESS or code != 200 or body.size() == 0:
			push_warning("[KsanaDock] Audio download failed (result=%d, code=%d, size=%d) URL: %s" % [result, code, body.size(), optimized_url])
			return
		...
		if body.size() > 10:
			...
			if "<!doctype" in head or "<html" in head:
				push_warning("[KsanaDock] Downloaded content is HTML, not audio. URL: %s" % optimized_url)
				return
		...
		if not file:
			push_error("[KsanaDock] Cannot write temp audio file: %s" % tmp_path)
			return
...
static func _play_file(path: String, url: String) -> void:
	...
	if data.size() == 0:
		push_warning("[KsanaDock] Audio file is empty: %s" % path)
		return
	...
	if is_wav:
		if path.ends_with(".mp3"):
			print("[KsanaDock] Detected WAV content in .mp3 file, using WAV loader.")
	...
	if stream:
		...
		push_warning("[KsanaDock] Could not create audio stream from: %s (Size: %d)" % [path, data.size()])
...
static func _load_wav_from_buffer(bytes: PackedByteArray) -> AudioStreamWAV:
	...
	if format != 1: # Only PCM supported
		push_error("[KsanaDock] Only PCM WAV is supported.")
		return null
	...
	if bits == 8:
		...
		push_error("[KsanaDock] Unsupported WAV bits per sample: %d" % bits)
		return null
