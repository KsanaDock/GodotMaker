...
func _build_tabs() -> void:
	...
	for tab_def in TABS:
		...
		btn.text = " " + tab_label
		btn.icon = load("res://addons/ksanadock/icons/ui/" + tab_def["icon"])
...
func _update_user_info() -> void:
	...
	var plan_icon_path := "res://addons/ksanadock/icons/ui/send.svg" # 默认为 Free 方案使用飞行纸飞机 (send.svg)
	match plan.to_lower():
...
func _set_auth_btn_mode(logged_in: bool) -> void:
	...
	if logged_in:
		...
		_auth_btn.icon = load("res://addons/ksanadock/icons/ui/key.svg")
		_auth_btn.add_theme_stylebox_override("normal", KPalette.btn_primary())
		_auth_btn.add_theme_stylebox_override("hover", KPalette.btn_primary_hover())
...
func _create_asset_card(asset: Dictionary) -> PanelContainer:
	...
	placeholder.texture = load("res://addons/ksanadock/icons/ui/" + icon_name)
	...
	if _active_tab == KAssetClient.TYPE_MUSIC:
		...
		play_btn.icon = load("res://addons/ksanadock/icons/ui/play.svg")
		...
		if audio_url != "":
			play_btn.pressed.connect(func():
				...
				if KAudioLoader.is_playing_url(audio_url):
					play_btn.icon = load("res://addons/ksanadock/icons/ui/pause.svg")
				else:
					play_btn.icon = load("res://addons/ksanadock/icons/ui/play.svg")
			)
