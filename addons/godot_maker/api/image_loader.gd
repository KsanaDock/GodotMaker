...
static func load_texture(url: String, callback: Callable) -> void:
	...
	http.request_completed.connect(func(result: int, code: int, _headers: PackedStringArray, body: PackedByteArray):
		...
		if result != HTTPRequest.RESULT_SUCCESS:
			if result == HTTPRequest.RESULT_CANT_CONNECT or result == HTTPRequest.RESULT_TIMEOUT or result == 4:
				...
				print("[GodotMaker] Image download skipped (result=%d, possibly network/SSL issue) for URL: %s" % [result, optimized_url])
			else:
				push_warning("[GodotMaker] Image download failed (result=%d) for URL: %s" % [result, optimized_url])
			callback.call(null)
			return
		
		if code != 200:
			push_warning("[GodotMaker] Image HTTP %d for URL: %s" % [code, optimized_url])
			callback.call(null)
			return
		...
		if body.size() > 10:
			...
			if "<!doctype" in head or "<html" in head:
				push_warning("[GodotMaker] Downloaded content is HTML, not an image. URL: %s" % optimized_url)
				callback.call(null)
				return
				
		var format := _identify_image_format(body)
		if format == ImageFormat.NONE:
			push_warning("[GodotMaker] Downloaded data is not a recognizable image format. URL: %s" % optimized_url)
			callback.call(null)
			return
		...
		match format:
			...
			ImageFormat.GIF:
				...
				push_warning("[GodotMaker] GIF format detected but not currently supported for direct buffer loading. URL: %s" % optimized_url)
				callback.call(null)
				return
			
		if error != OK:
			push_error("[GodotMaker] Failed to decode image (format=%d, error=%d) URL: %s" % [format, error, optimized_url])
			callback.call(null)
			return
	...
	if err != OK:
		push_error("[GodotMaker] Failed to start image request (error=%d) URL: %s" % [err, optimized_url])
		http.queue_free()
		callback.call(null)
